import pygame
pygame.mixer.init()
pygame.mixer.music.load('digital-love-127441.mp3')
...
class Game(Ursina):
    def __init__(self):
        ...
        Light(type='ambient', color=(0.2, 0.1, 0.2, 1))
        Light(type='directional', color=(0.1, 0.1, 0.2, 1), direction=(0, 0, 1))
        self.MAP_SIZE = 20
        self.new_game()
    ...
    def check_game_over(self):
        ...
        if 0 < snake[-1][0] < self.MAP_SIZE and 0 < snake[-1][1] < self.MAP_SIZE and len(snake) == len(set(snake)):
                return
        print_on_screen('Ти Програв!', position=(-0.7, 0.1), scale=10, duration=1)
        self.snake.direction = Vec3(0, 0, 0)
        self.snake.permissions = dict.fromkeys(self.snake.permissions, 0)
        invoke(self.new_game, delay=1)

    def update(self):
        print_on_screen(f'Рахунок: {self.snake.score}', position=(-0.85, 0.45), scale=3, duration=1 / 20)
